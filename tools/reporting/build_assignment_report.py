...
def _architecture_section_lines(vector_payload: dict, diagram_ref: str) -> list[str]:
    ...
    return [
        ...
        "- á» táº§ng dÆ°á»›i, AI Agent lĂ m nhiá»‡m vá»¥ Ä‘iá»u phá»‘i guardrail, controlled router, gá»i web search, gá»i RAG vĂ  tráº£ Ä‘Ă¡p Ă¡n vá» Web UI hoáº·c API.",
        ...
        "- Tách 4 nhóm tri thức độc lập để route đúng ngữ cảnh trước khi generate.",
        ...
        "- Kho tri thức được tách thành `student_handbook_rag`, `academic_policy_rag`, `student_faq_rag`, `general_ictu_rag`.",
...
def _framework_section_lines() -> list[str]:
    return [
        ...
        "5. Route RAG: `_route_rag` gọi `route_rag_tool` để chọn `student_handbook_rag`, `academic_policy_rag`, `student_faq_rag` hoặc `general_ictu_rag`.",
        ...
        "10. General ICTU retrieval: dùng vector, RRF hybrid và lexical fallback cho câu hỏi tổng quát hoặc dữ liệu chưa đều.",
...
def _build_report_markdown(
    ...
    lines = [
        ...
        "- Hệ thống đã tách 4 nhóm tri thức độc lập để tránh trộn tất cả dữ liệu vào một kho duy nhất.",
